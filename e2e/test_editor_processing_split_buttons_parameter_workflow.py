...
def test_grouped_volume_and_speed_split_buttons_apply_local_values_without_changing_settings(
    ...
    _configure_ffmpeg(anki_mw, ffmpeg_config, volume_step_db=15, speed_step=1.5)
    ...
    try:
        ...
        click_selector(
            editor.web,
            '[data-testid="aqe-split-0-speed-preset-2"]',
            timeout=5.0,
        )
        ...
        _wait_for_status_flow(
            editor,
            lambda status: status["text"] == "Increased speed to x2.",
            timeout=10.0,
        )

        config = anki_mw.addonManager.getConfig(ADDON_NUMERIC_ID)
        assert config["volume_step_db"] == 15

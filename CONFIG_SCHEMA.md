...
| `enable_reviewer_editor` | boolean | Enables inline editor controls on reviewer cards |
...
| `visible_editor_buttons` | string array | Ordered list of configurable editor panel command buttons to render; removing a command hides that button |
| `editor_button_modes` | object | Per-command display mode map for configurable editor panel buttons: `icon` or `text` |
...
| `output_format` | string | Default target for Convert operations: `source`, `mp3`, `m4a`, `wav`, or `flac` |
| `size_reduction_mode` | string | Default Reduce Size split-button level: `gentle`, `normal`, or `aggressive` |

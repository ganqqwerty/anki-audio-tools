...
Anki desktop add-on for quick, non-destructive audio edits from the Anki Editor plus Browser batch audio operations and prosody visualization. Add-on runtime code lives in `addon/anki_audio_quick_editor/`.
...
After major refactors, package renames, config schema changes, hook changes, or architecture updates, run `/doc-maintain` in Claude Code or use the `$doc-maintain` Codex skill. The workflow should regenerate the architecture archive with `python3 scripts/dev.py graphs-archive` and use those machine-readable module, bridge, webview, and relationship catalogs to understand current connections before editing docs.
...
| `addon/anki_audio_quick_editor/templates/` | Ignored generated settings, editor, and Browser batch webview bundle output |
...
| `docs/graphs/` | Human-readable architecture diagrams generated by `scripts/dev.py graphs-*` |
| `docs/archive/architecture_diagrams/` | Date-stamped machine-readable architecture archives for LLM/doc audits      |

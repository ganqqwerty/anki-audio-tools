...
- [x] Additional cleanup completed: session types, reducer helpers, backend queueing, post-edit ready dispatch, learner projection helpers, and field projection effects were split out so `html-audio-session-machine.ts` and `html-audio-session-controller.ts` stay under the existing frontend architecture line budget.
- [x] Latest focused verification: `npm --prefix settings_ui test -- frontend-architecture.test.ts html-audio-session-machine.test.ts editor-inline.html-audio-session.integration.test.ts editor-inline.selection-playback.integration.test.ts editor-inline.playback.integration.test.ts` passes.
...
- [x] Readability review completed: reducer helper transitions live in `html-audio-session-machine-helpers.ts`, end/repeat field projection effects live in `html-audio-session-field-effects.ts`, and the remaining controller owns effect orchestration plus session-backed projection inputs.

...
def render_dpdfnet_audio(
    ...
    try:
        ...
        if output_path is None:
            output_path = Path(tempfile.mkstemp(prefix="aqe_dpdfnet_", suffix=".mp3")[1])
        ...
        if dpdfnet_result.returncode != 0:
            raise AudioProcessingError(
                _render_external_error_message(dpdfnet_result, "DPDFNet denoise failed.")
            )
...
def render_voice_only_audio(
    ...
    try:
        ...
        if output_path is None:
            output_path = Path(tempfile.mkstemp(prefix="aqe_voice_only_", suffix=".mp3")[1])
        ...
        prepare_cmd = build_spleeter_prepare_command(ffmpeg_path, source_path, input_wav)
        prepare_result = _run_recorded_external_command(
            prepare_cmd,
        ...
        spleeter_result = _run_recorded_external_command(
            spleeter_cmd,
        ...
        if spleeter_result.returncode != 0:
            raise AudioProcessingError(
                _render_external_error_message(spleeter_result, "Voice Only extraction failed.")
            )
        if not vocals_wav.is_file():
            raise AudioProcessingError("Voice Only extraction did not produce vocals.wav.")

        encode_cmd = build_mp3_encode_command(ffmpeg_path, vocals_wav, output_path)
        encode_result = _run_recorded_external_command(
            encode_cmd,
...
def _record_spleeter_failure(
    ...
    *,
    ...
    record_latest_spleeter_support_incident(
        ...
        accompaniment_model_path=(
            str(accompaniment_model_path) if accompaniment_model_path is not None else ""
        ),
        attempted_commands=attempted_commands,
    )

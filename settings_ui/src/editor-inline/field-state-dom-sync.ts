...
export function syncFieldStateToDom(ord: number, state: EditorFieldState): void {
  ...
  visualizer.dataset.anchorMs = String(state.cursor.anchorMs);
  visualizer.dataset.cursorMs = String(state.cursor.ms);
  visualizer.dataset.progressMs = String(state.cursor.progressMs);
  visualizer.dataset.playbackState = state.playback.state;
  visualizer.dataset.playbackEngine = state.playback.engine;
  visualizer.dataset.playbackStartMs = String(state.playback.startMs);
  visualizer.dataset.playbackEndMs = String(state.playback.endMs);

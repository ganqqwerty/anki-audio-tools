...
class _FakeRecordingController:
    ...
    def stop(self, *, on_completed: object, on_failed: object) -> None:
        ...
        if callable(on_completed):
            on_completed(recording_result_from_path(self.output_path, generation=self.generation))


def test_fake_recording_controller_completes_with_generation_and_wav_path(tmp_path: Path) -> None:
    ...
    assert started == [42]
    assert failures == []
    assert completed == [RecordingResult(path=wav_path, generation=42, duration_ms=None)]
...
def test_recording_result_from_path_rejects_missing_or_empty_files(tmp_path: Path) -> None:
...
def test_recording_result_from_path_includes_elapsed_duration(tmp_path: Path) -> None:
    ...
    result = recording_result_from_path(wav_path, generation=3, started_at=10.0, stopped_at=11.25)

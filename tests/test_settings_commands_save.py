...
def test_settings_cancel_rejects_dialog() -> None:
    ...
    assert handle_settings_command(_bridge_command("settings.cancel"), eval_fn, dialog) is True
...
def test_unknown_command_returns_false() -> None:
    ...
    assert handle_settings_command(_bridge_command("unknown.command"), eval_fn, dialog) is False

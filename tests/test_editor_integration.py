...
def test_bridge_defers_pending_payload_from_web_callback(tmp_path: Path, monkeypatch) -> None:
    ...
    _SESSIONS[editor] = session
    rendered: dict[str, AudioEditState] = {}
    monkeypatch.setattr("anki_audio_quick_editor.editor_integration._session_and_source", lambda _editor: (session, source))
    monkeypatch.setattr("anki_audio_quick_editor.editor_integration._config", lambda _editor: {})
    monkeypatch.setattr(
        "anki_audio_quick_editor.editor_integration._render_and_replace_async",
...
def test_busy_session_rejects_processing_command(tmp_path: Path, monkeypatch) -> None:
    ...
    render = MagicMock()
    monkeypatch.setattr("anki_audio_quick_editor.editor_integration._session_and_source", lambda _editor: (session, source))
    monkeypatch.setattr("anki_audio_quick_editor.editor_integration._render_and_replace_async", render)

...
- Processing presets are defined in Settings, serialized through generated contracts, and passed to both the inline editor config and Browser batch initial state. Keep the Settings preset constructor, editor preset split button, Browser preset controls, and Python preset validation in sync when the preset schema changes.
